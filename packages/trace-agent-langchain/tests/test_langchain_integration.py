"""Integration tests for LangChain callback with the TraceAgent backend.
...
chain with the TraceAgent callback, and verify that events are persisted.
"""
...
class TestLangChainCallbackIntegration:
    def test_chain_events_are_persisted(self, test_client) -> None:
        ...
        from trace_agent_sdk import RunSession, TraceAgentClient

        TRACE_AGENT_client = TraceAgentClient(base_url="", http_client=client)
        # Manually construct a RunSession since we bypass start_run()
        run_session = RunSession(
            client=TRACE_AGENT_client,
            run_id=run_id,
            agent_name="test-agent",
            goal="test",
        )
        callback = TraceAgentLangChainCallback(run_session)
