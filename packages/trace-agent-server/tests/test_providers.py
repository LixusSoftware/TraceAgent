...
from trace_agent_server.main import build_provider_registry
from trace_agent_server.schemas import ToolDefinition
...
class TestOpenAIProviderAdapter:
    def test_generate_turn_with_tools(self) -> None:
        ...
        with patch("trace_agent_server.services.provider.OpenAI") as MockClient:
    ...
    def test_missing_config(self) -> None:
        ...
        adapter = OpenAIProviderAdapter(settings)
        with pytest.raises(RuntimeError, match="TRACE_AGENT_OPENAI_API_KEY"):
...
class TestAnthropicProviderAdapter:
    def test_generate_turn_with_tools(self) -> None:
        ...
        with patch("trace_agent_server.services.provider.Anthropic") as MockClient:
    ...
    def test_missing_config(self) -> None:
        ...
        adapter = AnthropicProviderAdapter(settings)
        with pytest.raises(RuntimeError, match="TRACE_AGENT_ANTHROPIC_API_KEY"):
            adapter.generate_turn(
    ...
    def test_sdk_not_installed(self) -> None:
        settings = Settings(anthropic_api_key="test-key", anthropic_base_url=None)
        adapter = AnthropicProviderAdapter(settings)
        with patch("trace_agent_server.services.provider.Anthropic", None):
...
class TestGoogleProviderAdapter:
    def test_generate_turn_with_tools(self) -> None:
        ...
        with patch("trace_agent_server.services.provider.genai") as MockGenAI:
    ...
    def test_missing_config(self) -> None:
        ...
        adapter = GoogleProviderAdapter(settings)
        with pytest.raises(RuntimeError, match="TRACE_AGENT_GOOGLE_API_KEY"):
            adapter.generate_turn(
    ...
    def test_sdk_not_installed(self) -> None:
        settings = Settings(google_api_key="test-key")
        adapter = GoogleProviderAdapter(settings)
        with patch("trace_agent_server.services.provider.genai", None):
...
class TestKimiProviderAdapter:
    ...
    def test_missing_config(self) -> None:
        ...
        adapter = KimiProviderAdapter(settings)
        with pytest.raises(RuntimeError, match="TRACE_AGENT_KIMI_API_KEY"):
            adapter.generate_turn(
...
class TestBuildProviderRegistry:
    ...
    def test_only_openai(self) -> None:
        settings = Settings(openai_api_key="okey", openai_base_url=None, lmstudio_base_url=None)
        registry = build_provider_registry(settings)
