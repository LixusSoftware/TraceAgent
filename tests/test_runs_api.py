...
def build_test_clients(tmp_path, provider) -> tuple[TestClient, VisorClient]:
    settings = Settings(
    ...
    sdk_client = VisorClient(base_url="http://testserver", http_client=test_client)
    return test_client, sdk_client


def execute_similarity_run(
    sdk_client: VisorClient,
    *,
    agent_name: str,
    goal: str,
