"""LangChain callback handler that streams execution events into Visor Agentico.
...
class VisorLangChainCallback(BaseCallbackHandler):
    """Record LangChain execution as Visor observations.

    The callback translates LangChain lifecycle events into HTTP POSTs to the
    Visor backend.  If the backend is unreachable the callback logs a warning
    ...
    def _post_observations(self, observations: list[dict[str, Any]]) -> None:
        """Send observations to the Visor backend."""
        if not observations:
            return
        try:
    ...
    def _post_tool_results(self, results: list[dict[str, Any]]) -> None:
        """Send tool results to the Visor backend."""
        if not results:
            return
        try:
            self.run_session.client._request(
